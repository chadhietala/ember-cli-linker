...
Linker.prototype._isLegacy = function(importName) {
  if (this.legacyFilesToAppend.length > 0) {
    return this.legacyFilesToAppend.some(function(legacyMeta) {
      return legacyMeta.files && legacyMeta.files.indexOf(importName) > -1;
    });
  }
...
Linker.prototype.moveEngines = function() {
  ...
  engineNames.forEach(function(engine) {
    engines[engine].map(byFileName).forEach(function(file) {
      ...
      if (fs.existsSync(sourcePath) && !fs.existsSync(enginesPath)) {
        fs.outputFileSync(enginesPath, fs.readFileSync(sourcePath));
        fs.removeSync(sourcePath);
      } else if (legacyFile) {
        if (legacyFilesSync.indexOf(legacyFile) < 0) {
          enginesPath = path.join(this.outputPath, 'engines', engine, path.basename(legacyFile));
          fs.outputFileSync(enginesPath, fs.readFileSync(path.join(process.cwd(), legacyFile)));
        } else {
          debug('Skipping sync as ' + legacyFile + ' satisfies the ' + file + ' dependency.' );
        }
      }
    }, this);
  }, this);
  ...
  engineNames.map(function(engine) {
    return path.join(this.outputPath, engine);
  }, this).filter(function(initialEnginePath) {
    return fs.existsSync(initialEnginePath);
  }).forEach(function(initialEnginePath) {
    fs.removeSync(initialEnginePath);
  });
};

Linker.prototype._lookupLegacyFile = function(file) {
  var fileMeta = head(this.legacyFilesToAppend.filter(function(legacyMeta) {
    return legacyMeta.files && legacyMeta.files.indexOf(file.replace(path.extname(file), '')) > -1;
  }));
