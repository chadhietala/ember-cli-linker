...
module.exports = CoreObject.extend({
  init: function(inputTrees, options) {
    ...
    this.whitelist = ['__packager__'].concat(this.options.whitelist);
  ...
  /**
   * @private
   * Returns the input trees by their name
   *
   * @return {Array} An array of tree names
   */
  _treeNames: function() {
  ...
  /**
   * @private
   * Creates an pairs which represent the stable source path which Broccoli sets up and the name of the tree.
   * @return {Array} An array of pairs [name, srcDir]
   */
  ...
  /**
   * Decorates the tree desciptors with the source directory that is given by broccoli and also sets the relative paths for the tree.
   * @return {Nil}
   */
  ...
  /**
   * Hashes the contents of each dep graph and is used as the initial cache in determining if the graph has mutated.
   * @return {Object} An object that is used to derive cache hit / miss
   */
  hashGraphs: function(namesAndPaths) {
    var graphHashes = {};

    namesAndPaths.forEach(function(treeNameAndPath) {
      var name = treeNameAndPath[0];
      var srcDir = treeNameAndPath[1];
      var depGraphPath = path.join(srcDir, 'dep-graph.json');
      var isWhiteListed = this.whitelist.indexOf(name) > -1;
      ...
      if (fs.existsSync(depGraphPath)) {
        ...
        denormalizedGraph = null;
      } else  {
        throw new Error('Cannot find hash for ' + name);
      }

      // TODO
      // Addons that don't have javascript should never enter the linker,
      // but there is currently no way to determine this. This evicts addons
      // that do not have a real dep-graph
      if (denormalizedGraph && stringify(denormalizedGraph) !== '{}') {
        // TODO
        // We don't really need to hash, a string comparison should be fine
        graphHashes[name] = {
          name: name,
          hash: helpers.hashStrings([stringify(denormalizedGraph)]),
  ...
  diffGraph: function() {
    ...
    var incomingHashes = this.hashGraphs(AllDependencies.getPackagePathsAndNames());
  ...
  rebuild: function() {
    ...
    if (this.initialBuild) {
      this.graphHashes = this.hashGraphs(this._eligibleTrees());
  ...
  /**
   * Filters out directories by checking if the string ends with a slash.
   * This a gurantee is provided to use via walk-sync.
  ...
  syncForwardNonGraphAssets: function() {
    ...
    zip(this.inputTrees, this.inputPaths).filter(function(treeAndPath) {
      ...
      var name = tree.name;
      return !!name;
    }, this).forEach(function(treeAndPath) {
      var srcDir = treeAndPath[1];
      var name = treeAndPath[0].name;
      var relativePaths = walkSync(srcDir).filter(function(relativePath) {
        return relativePath.slice(-1) !== '/';
      });

      function syncNonGraphAsset(relativePath) {
      ...
      // Whitelist packages just get synced
      if (this.whitelist.indexOf(name) > -1) {
        relativePaths.forEach(syncNonGraphAsset);
      } else {
        // Packages that are going to be resolved should sync forward
        // any non-graph assets
        relativePaths.filter(function(relativePath) {
          var isDepgraph = relativePath.indexOf('dep-graph.json') > -1;
          var isJsFile = path.extname(relativePath) === '.js';
          if (isDepgraph || isJsFile) {
            return false;
          }

          return true;
        }).forEach(syncNonGraphAsset);
      }
    }, this);
  },
   ...
   * @return {Nil}
   */
  syncForwardRoot: function(packageName) {
  ...
  resolve: function(packageNames) {
    ...
    return RSVP.Promise.all(packageNames.map(function(packageName) {
      ...
      return RSVP.Promise.all(Object.keys(this.resolvers).map(function(resolver) {
        ...
        if (resolveLater) {
          return resolveLater.call(this.resolvers[resolver], this.outputPath, this.treeDescriptors);
        }
      }, this));
    }.bind(this));
  },
   ...
   */
  selectResolution: function(packageName, imports, importer) {
