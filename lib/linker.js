 ...
 * @return {[type]}              [description]
 */
Linker.prototype._filterOutDiretory = function(relativePath) {
 ...
 */
Linker.prototype.syncForwardNonGraphAssets = function() {
  var self = this;

  this.orderedDescriptors.forEach(function(desc, i) {
    var srcDir = this.inputPaths[i];
    var name = desc.name;
    var relativePaths = walkSync(srcDir).filter(this._filterOutDiretory);

    function syncNonGraphAsset(relativePath) {
      var source = srcDir + path.sep + relativePath;
      var destination = self.outputPath + path.sep + relativePath;
      syncForwardDependencies({
        source: source,
 ...
 * @return {Nil}
 */
Linker.prototype.syncForwardRoot = function(packageName) {
...
Linker.prototype.selectResolution = function(packageName, imports, importer) {
  return mapSeries(imports, function(importee) {
    ...
    if (isLegacy) {
      this.legacyImportsSeen = uniq(this.legacyImportsSeen.concat(importee));
