...
module.exports = CoreObject.extend({
  ...
  rebuild: function() {
    ...
    if (this.initialBuild) {
      ...
      diffs = this.entries.map(this._entryToDiff.bind(this));
  ...
  syncForwardRoot: function(entry, relativePaths) {
  ...
  _entryToDiff: function(entry) {
    var hashEntry = this.graphHashes[entry];
    return {
      name: hashEntry.name,
      hash: hashEntry.hash,
