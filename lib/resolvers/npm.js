...
module.exports = {
  ...
  _gatherModuleBaseDirs: function(basedirs) {
    return basedirs.map(function(basedir) {
      var deps = Object.keys(fs.readJSONSync(path.join(basedir, 'package.json')).dependencies);
      var newBaseDirs = this._concretePaths(deps, basedir);
      return this._gatherModuleBaseDirs(newBaseDirs);
    }, this);
  ...
  moduleDirs: function(dependentEdges, descriptors) {
    var moduleBaseDirs = dependentEdges.map(function(edge) {
      ...
      return this._moduleBaseDir(resolvedPath, importName);
    }, this);

    return flatten(this._gatherModuleBaseDirs(moduleBaseDirs)).concat(moduleBaseDirs);
  },
  /**
