...
module.exports = {
  ...
  syncNodeModules: function(nodeModulePaths) {
    ...
    nodeModulePaths.forEach(function(dirPath) {
   ...
   */
  _gatherModuleBaseDirs: function(basedirs) {
    return basedirs.map(function(basedir) {
  ...
  _concretePaths: function(deps, basedir) {
    ...
    return deps.map(function(dep) {
      var depPath = path.join(nodeModulesPath, dep);
  ...
  moduleDirs: function(dependentEdges, descriptors) {
    ...
    return uniq(flatten(
      this._gatherModuleBaseDirs(moduleBaseDirs, packageNames)
    ).concat(moduleBaseDirs));
  },
  /**
   ...
   */
  resolveLater: function(destDir, descriptors, linker) {
    ...
    this.tmpBrowserify = path.join(linker.cachePath, 'tmpBrowserify');
    this.browserifyStaging = path.join(linker.cachePath, 'browserifyStaging');
    fs.mkdirsSync(this.tmpBrowserify);
    fs.mkdirsSync(this.browserifyStaging);

    this.browserifiedPath = path.join(this.tmpBrowserify, this.bundleName);
    var dirs = this.moduleDirs(dependentEdges, descriptors);
    this.syncNodeModules(dirs);
