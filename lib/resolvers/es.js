...
module.exports = {
  ...
  /**
   * Merges the babel options passed in with the defaults for the
   * es resolver
   * @param  {String} importName The name of import we are calculating options for
   * @param  {String} [basePath]   The module basepath
   * @return {Object}            The merged options
   */
  _babelOptions: function(importName, basePath) {
    return defaults(this.babelOptions, {
      modules: 'amd',
      moduleIds: true,
      moduleId: importName,
      filename: importName,
  ...
  compileThroughCache: function(source, importName, basePath) {
    ...
    if (!cache || cache.src !== source) {
      return new RSVP.Promise(function(resolve) {
        var babelOptions = self._babelOptions(importName, basePath);
        var transpiled = babel.transform(source, babelOptions);
  ...
  resolveImport: function(nodeModulesPath, packageName, importName, importer) {
    ...
    return this.compileThroughCache(
      ...
      AllDependencies.setEdge(importer, [relativePath]);

      AllDependencies.addNode(relativePath, {
        source: tmpPath,
        destination: destinationPath,
  ...
  /**
   * Removes fields from the babel options that are specific are either broccoli-babel-transpiler or options where the es resolver needs to take precedence.
   * @param  {Object} dirtyBabelOptions Babel options with broccoli specific options
   * @return {Object}                   The sanitized babel options
   */
  _sanitizeBabelOptions: function(dirtyBabelOptions) {
    var optionsToEvict = [
      'filterExtensions',
      'browserPolyfill',
      'exportModuleMetadata',
      'resolveModuleSource'
    ];
    var babelOptions = {};

    Object.keys(dirtyBabelOptions).filter(function(option) {
      return optionsToEvict.indexOf(option) < -1;
    }).forEach(function(option) {
      babelOptions[option] = dirtyBabelOptions[option];
    });
  ...
  resolve: function(destDir, importInfo, linker) {
    ...
    var babelOptions = linker.options && linker.options.babel ? linker.options.babel : {};
    this.babelOptions = this._sanitizeBabelOptions(babelOptions);
