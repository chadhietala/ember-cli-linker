...
var AllDependencies = {
  ...
  graphForEngines: function() {
    ...
    engines['shared-by-all'] = sharedDeps;

    this.roots.forEach(function(root, i) {
      engines[root] = boundaries[i];
    }, this);
  ...
  _graphBoundaries: function(roots) {
    return roots.map(function(root) {
      ...
      var rootNodes = subgraph.filter(function(node) {
        return this.getNodeMeta(node).packageName === root;
      }, this);

      var localNodes = uniq(flatten(rootNodes.map(function(node) {
        return flatten(this._localNodes(roots, node, root));
      }, this)));
   ...
   */
  _localNodes: function(roots, node, root) {
    var rootsToCompare = roots.filter(function(r) {
      return r !== root;
    });

    return this.getOutwardEdges(node).filter(function(edge) {
      var packageName = this.getNodeMeta(edge.w).packageName;
