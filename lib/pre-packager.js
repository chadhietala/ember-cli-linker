...
module.exports = CoreObject.extend({
  ...
  _shouldResolveTree: function (pkgName, relativePaths) {
    if (pkgName === '__packager__') {
      relativePaths.splice(relativePaths.indexOf('__packager__/dep-graph.json'), 1);
      return false;
    }
  ...
  selectResolution: function(entry, pkgInfo) {
    ...
    return mapSeries(uniq(imports), function(imprt) {
      var importInfo = this.getImportInfo(imprt, entry);
  ...
  getImportInfo: function(imprt, entry) {
    if (imprt.indexOf(entry + '/') > -1) {
      return { pkg: entry, name: imprt, type: 'addon' }
    } else {
      return getImportInfo(imprt);
    }
  }
});