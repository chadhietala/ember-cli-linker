...
module.exports = CoreObject.extend({
  ...
  resolveEntries: function(srcDir, destDir) {
    ...
    return RSVP.Promise.all(this.entries.map(function(entry) {
      ...
      if (!cacheValid) {
        ...
        return RSVP.Promise.all(self.selectResolution(srcDir, destDir, self.flattenEntryImports(entry, entryDepGraphPath)));
      }
  ...
  npm: function(srcDir, destDir, imprt) {
    if (this.f === 0) {
      this.f++;
  ...
  selectResolution: function(srcDir, destDir, imports) {
    var self = this;
    return imports.toJS().map(function(imprt) {
      var importType = self.importType(imprt);

      if (self.resolutionTypes.indexOf(importType) > -1) {
        // return self.npm(srcDir, destDir, imprt.split(':')[1]);
  ...
  defaultResolver: function(srcDir, destDir, imprt) {
    ...
    if (!this.checkCache(entry, depGraph)) {
      ...
      return RSVP.Promise.all(this.selectResolution(srcDir, destDir, AllDependencies.for(imprt)));
    }
