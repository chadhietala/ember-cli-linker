...
module.exports = CoreObject.extend({
  init: function(inputTree, options) {
    ...
    this.resolutionTypes.forEach(function(type) {
      self.resolvers[type] = require('./resolvers/' + type);
    });
  ...
  resolveEntries: function(srcDir, destDir) {
    ...
    return RSVP.Promise.all(this.entries.map(function(entry) {
      ...
      if (!cacheValid) {
        ...
        return self.selectResolution(srcDir, destDir, self.flattenEntryImports(entry, entryDepGraphPath));
      }
  ...
  selectResolution: function(srcDir, destDir, imports) {
    ...
    return mapSeries(mori.toJs(mori.set(imports)), function(imprt) {
      ...
      if (self.resolutionTypes.indexOf(importType) > -1) {
        return self.resolvers[importType].resolve(srcDir, destDir, imprt.split(':')[1]);
      }
      
      return self.defaultResolver(srcDir, destDir, imprt);
    });
  },
  ...
  defaultResolver: function(srcDir, destDir, imprt) {
    ...
    if (!this.checkCache(entry, depGraph)) {
      ...
      return this.selectResolution(srcDir, destDir, AllDependencies.for(imprt));
    }
