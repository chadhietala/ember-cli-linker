...
function flatMap(sequence, fn) {
  return mori.set(mori.flatten(mori.map(fn, sequence)));
}
...
module.exports = CoreObject.extend({
  ...
  resolveEntries: function(srcDir, destDir) {
    ...
    return RSVP.Promise.all(this.entries.map(function(entry) {
      ...
      var cacheValid = checkCache(entry, entryDepGraphPath);

      if (!cacheValid) {
        // Sync the entry
        paths.filter(this._isEntryFiles(entry)).forEach(function(relativePath) {
  ...
  flattenEntryImports: function(entry) {
    return flatMap(mori.seq(mori.keys(AllDependencies.for(entry))), function(file) {
  ...
  selectResolution: function(srcDir, destDir, imports) {
    return mapSeries(mori.toJs(mori.set(imports)), function(imprt) {
      var importType = this.importType(imprt);

      if (this.resolutionTypes.indexOf(importType.type) < 0) {
        throw new Error('You do not have a resolver for '+ importType.type + ' types.');
      }

      return this.resolvers[importType.type].resolve(
        srcDir, destDir, importType.id, this
      );
