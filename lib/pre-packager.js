...
module.exports = CoreObject.extend({
  ...
  selectResolution: function(srcDir, destDir, imports) {
    var self = this;
    return RSVP.Promise.all(imports.toJS().map(function(imprt) {
      var importType = self.importType(imprt);
      if (self.resolutionTypes.indexOf(importType) > -1) {
        self.npm(srcDir, destDir, imprt.split(':')[1]);
  ...
  defaultResolver: function(srcDir, destDir, imprt) {
    var self = this;
    return new RSVP.Promise(function(resolve) {
      var entry = self._getEntry(imprt);
      ...
      if (!self.checkCache(entry, depGraph)) {
        ...
        self.selectResolution(srcDir, destDir, AllDependencies.for(imprt));
      }
