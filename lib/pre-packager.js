...
module.exports = CoreObject.extend({
  ...
  flattenEntryImports: function(entry) {
    ...
    var fileImports = uniq(flatMap(Object.keys(AllDependencies.for(entry)), function(file) {
      var files = AllDependencies.for(file);
      _graph.graph[file] = files;
      _graph.pkg = files.pkg;
      return files.fileImports;
    }));

    _graph.flattenedImports = fileImports;
  ...
  selectResolution: function(srcDir, destDir, imports) {
    var pkg = imports.pkg;
    var flattenedImports = imports.flattenedImports;
    var graph = imports.graph;

    console.log(flattenedImports);
    
    return mapSeries(uniq(flattenedImports), function(imprt) {
      ...
      var cacheItem = { import: importInfo.id, dependent: imports.file };
      if (!type) {
        type = this.importCache[importInfo.type] = {};
      }

      if (!type[importInfo.pkg]) {
        type[importInfo.pkg] = [cacheItem];
      } else {
        type[importInfo.pkg].push(cacheItem);
      }
